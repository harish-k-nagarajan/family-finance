...
function InvestmentModal({ investment, users, householdId, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (investment) {
        ...
        await db.transact(
          db.tx.investments[investment.id].update({
      ...
      // Show success toast immediately (investment creation succeeded)
      showToast(`Investment ${investment ? 'updated' : 'added'} successfully`, 'success');
      ...
      try {
        ...
        // Don't show error to user - investment creation was successful
      }
    } catch (error) {
      console.error('Failed to save investment:', error);
      showToast(
        `Failed to ${investment ? 'update' : 'add'} investment. Please try again.`,
        'error'
      );
    } finally {
      setIsLoading(false);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                <option value="401k" className="bg-white dark:bg-navy-800">
                  401(k)
                </option>
                <option value="IRA" className="bg-white dark:bg-navy-800">
                  Traditional IRA
                </option>
                <option value="Roth IRA" className="bg-white dark:bg-navy-800">
                  Roth IRA
                </option>
                <option value="Taxable" className="bg-white dark:bg-navy-800">
                  Taxable Brokerage
                </option>
                <option value="HSA" className="bg-white dark:bg-navy-800">
                  HSA
                </option>
                <option value="529" className="bg-white dark:bg-navy-800">
                  529 Plan
                </option>
              </select>
            </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="px-6 py-2 rounded-lg bg-gradient-to-r from-teal-500 to-purple-500 text-white font-medium hover:opacity-90 transition-opacity disabled:opacity-60 disabled:cursor-not-allowed min-w-[140px]"
            >
